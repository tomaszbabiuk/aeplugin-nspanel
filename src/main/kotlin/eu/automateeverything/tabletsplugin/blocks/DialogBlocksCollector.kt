...
@Suppress("unused")
@Extension
class DialogBlocksCollector(private val repository: Repository) : BlockFactoriesCollector {

    override fun collect(thisDevice: Configurable?): List<BlockFactory<*>> {
        return collectStaticBlocks() + listOf(DialogOptionsBlockFactory())
    }

    private fun collectStaticBlocks() = listOf(OptionBlockFactory(), ShowDialogBlockFactory())
}
